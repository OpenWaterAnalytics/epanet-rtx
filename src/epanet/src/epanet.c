...
/* Free temporary linked lists used for Patterns & Curves */
   ...
   freeTmplist(Coordlist); //06.02.2010 woohn
...
int DLLEXPORT ENgetcontrol(int cindex, int *ctype, int *lindex,
              double *setting, int *nindex, double *level)
/*----------------------------------------------------------------
...
int  DLLEXPORT ENgetoption(int code, double *value)
/*----------------------------------------------------------------
...
*/
{
   *value = 0;
   ...
   switch (code)
   {
      ...
      case EN_NEXTEVENT:    *value = Hstep;     // find the lesser of the hydraulic time step length, or the time to next fill/empty
                            tanktimestep(value); 
                            break;
   }
   return(0);
...
int  DLLEXPORT  ENsetpattern(int index, double *f, int n)
/*----------------------------------------------------------------
...
int  DLLEXPORT  ENsetpatternvalue(int index, int period, double value)
/*----------------------------------------------------------------
**  Input:   index  = time pattern index
...
{
   if (!Openflag) return(102);
  // override check for active simulation (sph)
...
/* Check that file names are not identical */
  // bug here ( TODO ) -- if i don't want a report, then f2 === f3
...
*/
{
   ...
   Curve    = NULL;
   Control  = NULL;
   Coord    = NULL;   //06.02.2010 woohn

   X        = NULL;
   Patlist  = NULL;
   Curvelist = NULL;
   Coordlist = NULL;	//06.02.2010 woohn
   Adjlist  = NULL;
   Aii      = NULL;
   Aij      = NULL;
   F        = NULL;
...
/* controls, demands, time patterns, & operating curves */
   if (!errcode)
   {
      ...
      Coord   = (Scoord *)   calloc(MaxNodes+1,  sizeof(Scoord)); //06.02.2010 woohn
      ...
      ERRCODE(MEMCHECK(Coord));  //06.02.2010 woohn
   }
